...
lumi_dict = {2015: 19.52, 2016: 16.81, 2017: 41.48, 2018: 59.83, 2024: 109.08}
...
class HeavyFlavBaseProducer(Module, object):
    ...
    def __init__(self, channel, **kwargs):
        ...
        if self._needsJMECorr:
            ...
            self.jetmetCorr = JetMETCorrector(year=self.year,jetType="AK4PFPuppi" if self.year == 2024 else "AK4PFchs", **self._jmeSysts)
        ...
        #add 2024 https://gitlab.cern.ch/cms-btv/btv-scale-factors/-/blob/master/2024_Summer24/wp/wp_database_btagging.yml?ref_type=heads
        self.btagUParTAK4B_WP_L = {2024: 0.0508}[self.year]
        self.btagUParTAK4B_WP_M = {2024: 0.1272}[self.year]
        self.btagUParTAK4B_WP_T = {2024: 0.4648}[self.year]
        self.btagUParTAK4B_WP_XT = {2024: 0.6298}[self.year]
        self.btagUParTAK4B_WP_XXT = {2024: 0.9739}[self.year]
    def beginJob(self):
        if self._needsJMECorr:
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.hasParticleNetProb = bool(inputTree.GetBranch(self._fj_name + '_ParticleNet_Xbb'))
        ...
        for idx in ([1, 2] if self._channel == 'qcd' else [1]):
            ...
            self.out.branch(prefix + "ParticleNetMasWiths_HccvsQCD", "F")
    ...
    def correctJetsAndMET(self, event):
        ...
        if self._needsJMECorr:
            ...
            self.jetmetCorr.correctJetAndMET(jets=event._allJets, lowPtJets=Collection(event, "CorrT1METJet"),
                                             ...
                                             defaultMET=METObject(event, "PuppiMET"),
        ...
        for fj in event._allFatJets:
            ...
            if self.year == 2024:
                try:
                    passes_jetid = self.fatjet_jetid_eval.evaluate(
                        fj.eta, fj.chHEF, fj.neHEF, fj.chEmEF, fj.neEmEF, fj.muEF,
                        float(fj.chMultiplicity), float(fj.neMultiplicity),
                        float(fj.chMultiplicity + fj.neMultiplicity)
                    )
                except Exception as e:
                    logger.warning(f"FatJet JetID eval failed: {e}")
                    passes_jetid = False
            else:
                passes_jetid = (fj.jetId & 2)
        ...
        for j in event._allJets:
            ...
            if self.year == 2024:
                try:
                    passes_jetid = self.jetid_eval.evaluate(
                        j.eta, j.chHEF, j.neHEF, j.chEmEF, j.neEmEF, j.muEF,
                        float(j.chMultiplicity), float(j.neMultiplicity),
                        float(j.chMultiplicity + j.neMultiplicity)
                    )
                except Exception as e:
                    logger.warning(f"AK4 JetID eval failed: {e}")
                    passes_jetid = False
            else:
                passes_jetid = (j.jetId & 4)
        ...
        for j in jets:
            if self._opts['run_tagger']:
                ...
                if self.hasParticleNetProb:
                    ...
                    #pnet legacy
                    j.pn_Leg_Xbb = j.particleNetLegacy_Xbb
                    j.pn_Leg_Xcc = j.particleNetLegacy_Xcc
                    j.pn_Leg_Xqq = j.particleNetLegacy_Xqq
                    j.pn_Leg_QCD = j.particleNetLegacy_QCD
                    #glopart3
                    j.glopart_Xbb = j.globalParT3_Xbb
                    j.glopart_Xcc = j.globalParT3_Xcc
                    j.glopart_Xqq = j.globalParT3_Xqq
                    j.glopart_QCD = j.globalParT3_QCD
            
            #pnet legacy
            j.pn_Leg_XbbVsQCD = convert_prob(j, ['Xbb'], ['QCD'], prefix='pn_Leg_')
            j.pn_Leg_XccVsQCD = convert_prob(j, ['Xcc'], ['QCD'], prefix='pn_Leg_')
            j.pn_Leg_XccOrXqqVsQCD = convert_prob(j, ['Xcc', 'Xqq'], ['QCD'], prefix='pn_Leg_')
            #glopart3
    ...
    def evalMassRegression(self, event, jets):
        for j in jets:
            if self._opts['run_mass_regression']:
                ...
                try:
                    j.regressed_mass_Legacy = j.particleNetLegacy_mass
                    j.regressed_mass_Glopart = j.mass* j.globalParT3_massCorrX2p* (1- j.rawFactor)
                except RuntimeError:
                    j.regressed_mass = 0
                    j.regressed_mass_Legacy =0
                    j.regressed_mass_Glopart =0
    ...
    def fillBaseEventInfo(self, event):
        ...
        if self.year in (2017, 2018, 2024):
    ...
    def fillFatJetInfo(self, event, fatjets):
        for idx in ([1, 2] if self._channel == 'qcd' else [1]):
            ...
            self.out.fillBranch(prefix + "regressed_mass_Legacy", fj.regressed_mass_Legacy)
            self.out.fillBranch(prefix + "regressed_mass_Glopart", fj.regressed_mass_Glopart)
            ...
            # taggers FOR 2024
            # ParticleNet-MD
            #self.out.fillBranch(prefix + "ParticleNet_Xbb", fj.pn_Xbb)
            #self.out.fillBranch(prefix + "ParticleNet_Xcc", fj.pn_Xcc)
            #self.out.fillBranch(prefix + "ParticleNet_Xqq", fj.pn_Xqq)
            self.out.fillBranch(prefix + "ParticleNet_QCD", fj.pn_QCD)
            ...
            # ParticleNet-legacy
            self.out.fillBranch(prefix + "particleNetLegacy_Xbb", fj.pn_Leg_Xbb)
            self.out.fillBranch(prefix + "particleNetLegacy_Xcc", fj.pn_Leg_Xcc)
            self.out.fillBranch(prefix + "particleNetLegacy_Xqq", fj.pn_Leg_Xqq)
            self.out.fillBranch(prefix + "particleNetLegacy_QCD", fj.pn_Leg_QCD)
            self.out.fillBranch(prefix + "particleNetLegacy_XbbVsQCD", fj.pn_Leg_XbbVsQCD)
            self.out.fillBranch(prefix + "particleNetLegacy_XccVsQCD", fj.pn_Leg_XccVsQCD)
            self.out.fillBranch(prefix + "particleNetLegacy_XccOrXqqVsQCD", fj.pn_Leg_XccOrXqqVsQCD)

            

            # Fill GlobalParT v3 discriminants
